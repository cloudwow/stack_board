...
class _ItemCaseState extends State<ItemCase> with SafeState<ItemCase> {
  ...
  void initState() {
    ...
    if (_caseStyle.initSize != null) {
      _config.value.size = _caseStyle.initSize;
    }
  }
  ...
  void _scaleHandle(DragUpdateDetails dud) {
    ...
    Offset start =
        _config.value.offset! + Offset(-_caseStyle.iconSize / 2, _caseStyle.iconSize * 2);
      ...
      content = GetSize(
        onChange: (Size? size) {
          if (size != null && _config.value.size == null) {
            _config.value.size =
                Size(size.width + _caseStyle.iconSize + 40, size.height + _caseStyle.iconSize + 40);
            safeSetState(() {});
          }
        },
        child: content,
    ...
    return Positioned(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(
            color:
                _operatState == OperatState.complate ? Colors.transparent : _caseStyle.borderColor,
